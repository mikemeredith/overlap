...
Density estimation involves smoothing the information in the data, and the degree of smoothing is controlled by the argument \code{c} to the \code{densityPlot} function. Increasing \emph{c} above the default value of 1 gives a more `spiky' curve, reducing it gives a flatter curve, as shown in Figure~\ref{fig:smoothing}. The choice of \emph{c} affects the estimate of overlap, as we discuss below.

<<smoothing, fig=TRUE, width=6, height=6, include=FALSE, echo=FALSE>>=
par(mfrow=2:1)
densityPlot(tig2, rug=TRUE, c=5)
text(2, 0.09, "c = 5")
densityPlot(tig2, rug=TRUE, c=0.5)
text(2, 0.065, "c = 0.5")
@
\begin{figure}[h]
  \centering
...
% \begin{figure}[h]
  % \centering
  % \includegraphics{overlapDiagram.jpg}
  % \caption{\it The coefficient of overlapping of two density curves equals the area below both curves, shaded grey in this diagram.}
  % \label{fig:overlapDefn}
% \end{figure}



As shown in Figure~\ref{fig:tigerMacaque2}, the coefficient of overlapping, $\Delta$, is the area lying under \emph{both} of the density curves. (Remember that the area under a density curve is, by definition, 1.) Mathematically, if the two density curves are $f(x)$ and $g(x)$, this is:
...
The first, $\hat{\Delta}_1$, matches the definition in equation \eqref{eq:definition}, but in practice it is estimated numerically, taking a large number of values, $t_1, t_2, ..., t_T$, equally spaced between 0 and $2\pi$ and summing:

\begin{equation} \label{eq:Dhat1}
  \hat{\Delta}_1 \approx \frac{1}{T} \sum_i \textrm{min} \{ \hat{f}(t_i), \hat{g}(t_i) \}
\end{equation}
...
The terms $\hat{f}(.)$ and $\hat{g}(.)$ refer to the fitted kernel density functions, and as such they are affected by the choice of the smoothing constant, $c$. On the basis of simulations, \citet{RidoutLinkie2009} recommend using $c=1.25$ to estimate $\hat{\Delta}_1$, $c=1.0$ for $\hat{\Delta}_4$, and $c=0.25$ for $\hat{\Delta}_5$. These are the default values used in \texttt{overlap} functions.
...
In no case was $\hat{\Delta}_5$ found to be useful. It is unstable, in that small, incremental changes in the data produce discontinuous changes in the estimate, and it can give estimates greater than 1.
...
Both of these samples are > 75, so the $\hat{\Delta}_4$ estimate, \texttt{Dhat4} in the \R{} code, is the most appropriate, giving an estimate of overlap of \Sexpr{round(tigmac2est[2], 2)}.
...
In nonparametric bootstrapping we treat the existing sample as representative of the population and generate a large number of new samples by randomly resampling observations with replacement from the original sample. For the case of estimating activity patterns, this may not work very well: suppose our original sample for a nocturnal species has observations between 21:00 and 03:00; resampling will never yield an observation outside that range, while a fresh sample from nature may do so.

For parametric bootstrapping we begin with a probability distribution and then draw random simulated observations from this distribution. For overlap, our starting distribution is the kernel density fitted to the data (so perhaps we should describe this as ``semi-parametric"). Faced with the original sample between 21:00 and 03:00, most simulated observations would fall in the same range, but a few will fall outside.

 In the \texttt{overlap} package, we generate bootstrap samples with either \texttt{parResample} or \texttt{nonparResample}. For this example, we will generate just 1000 parametric bootstrap samples for tigers and macaques in Zone 2; for a real analysis 10,000 bootstrap samples would be better:

<<bootstrap1>>==
tig2boot <- parResample(tig2, 1000)
dim(tig2boot)
mac2boot <- parResample(mac2, 1000)
dim(mac2boot)
@
...
Now we pass these two matrices to \texttt{bootEst} to generate estimates of overlap from each pair of samples. We are only concerned with $\hat{\Delta}_4$, so we will suppress estimation of the others by setting \texttt{c = c(NA, 1, NA)}; this reduces the time needed.

<<bootstrap2>>==
tigmac2 <- bootEst(tig2boot, mac2boot, c = c(NA, 1, NA))  # takes a few seconds
dim(tigmac2)
BSmean <- colMeans(tigmac2)
BSmean
@

Note that the bootstrap mean, $\overline{BS}$, differs from $\hat{\Delta}$: \Sexpr{round(BSmean[2], 2)} versus \Sexpr{round(tigmac2est[2], 2)}. The difference, $\hat{\Delta} - \overline{BS}$ is the \emph{bootstrap bias}, and is a complication for confidence interval estimation.
...
The \texttt{perc} confidence interval is centred on the bootstrap mean, $\overline{BS}$. Adjusting \texttt{perc} to account for the bootstrap bias yields the \texttt{basic0} confidence interval, which is centred on $\hat{\Delta}$.

An alternative approach is to use the standard deviation of the bootstrap results, $(s_{BS})$, as an estimate of the spread of the sampling distribution, and then calculate the confidence interval as $\hat{\Delta} \pm z_{{\alpha}/2} s_{BS}$. Using $z_{0.025} = 1.96$ gives the usual 95\% confidence interval. This is \texttt{norm0} in the \texttt{bootCI} output.

The difference between the values of the \texttt{basic0} and \texttt{norm0} is usually negligible.
...
\texttt{bootCI} produces two further estimators: \texttt{basic} and \texttt{norm}. These are analogous to \texttt{basic0} and \texttt{norm0} but are centred on the bias-corrected estimator, $\tilde{\Delta}$. They match the \texttt{basic} and \texttt{norm} confidence intervals produced by \texttt{boot.ci} in package \texttt{boot}.

The coefficient of overlapping takes values between 0 and 1. All the confidence interval estimators except \texttt{perc} involve additive corrections which might result in values <~0 or >~1. This can be avoided by carrying out the corrections on a logistic scale and back-transforming. This is done by \texttt{bootCIlogit}:
...
We ran a large number of simulations with different true distributions and sample sizes (see \citet{RidoutLinkie2009} for details). For each scenario, we ran both parametric and nonparametric bootstraps, extracted all nine 95\% confidence intervals, and checked the coverage for each.

Each estimator gave a range of coverages. We looked for a method which gave median coverage closest to the nominal 95\% and all or most values above 90\%. This was satisfied by the \texttt{boot0} estimator with parametric bootstraps.

With small samples (smaller sample <~75) and $\Delta > 0.8$, coverage fell below 90\%, but none of the other options fared better.
...
\begin{itemize}
  ...
  \item Use a parametric bootstrap and do at least 1000 resamples, preferably 10,000.
...
Camera traps set along animal trails -- as they often are -- record instances of animals moving along trails. The resulting ``activity pattern" refers to walking on trails, and overlap indicates the extent to which two species are walking on trails at the same period of the day. A browsing herbivore or the carnivore stalking it are probably both ``inactive" by this definition.
