...
Both of these samples have more than 75 observations, so the $\hat{\Delta}_4$ estimate, \texttt{Dhat4} in the \R{} code, is the most appropriate, giving an estimate of overlap of \Sexpr{round(tigmac2est[2], 2)}.
