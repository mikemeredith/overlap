overlapEst <-
function( A, B, kmax=3, c=NULL, n.grid=128) {
  ...
  #   c : smoother adjustment; either a single value used for all 3 Deltas,
  #         or a vector of 3 different values; a NA value in c means that
  #         the corresponding Dhat will not be estimated.
  ...
  if(!is.na(bw0.A) && !is.na(bw0.B))  {  # if no Uniroot Error
    if(!is.na(c[1]))  {  # Do Dhat1
      dA.current <- 1
      dA <- densityFit(A, grid, bw0.A * c[1])
      dB <- densityFit(B, grid, bw0.B * c[1])
    ...
    allObs <- c(A, B)
    n <- length(allObs)
    if(!is.na(c[2]))  {   # Get kernel densities for Dhat4
      if(n > (n.grid*1.1)) {  # Use interpolation
        if(is.na(c[1]) | !identical(all.equal(c[1], c[2]), TRUE))  { 
          dA.current <- 2
          dA <- densityFit(A, grid, bw0.A * c[2])
          dB <- densityFit(B, grid, bw0.B * c[2])
        }
        dAx <- approx(grid, dA, allObs)$y
        dBx <- approx(grid, dB, allObs)$y
      } else {
        dAx <- densityFit(A, allObs, bw0.A * c[2])
        dBx <- densityFit(B, allObs, bw0.B * c[2])
      }
      out[2] <- (mean(pmin(1, dBx[1:n1] / dAx[1:n1])) +   # Dhat4
                 mean(pmin(1, dAx[(n1+1):n] / dBx[(n1+1):n]))) / 2
    }
    if(!is.na(c[3]))  {
      if(is.na(c[2]) | !identical(all.equal(c[2], c[3]), TRUE))  { 
        # Do new kernel densities for Dhat5
        if(n > (n.grid*1.1)) {  # Use interpolation
          if(is.na(dA.current) | !identical(all.equal(c[dA.current], c[3]), TRUE))  { 
            dA <- densityFit(A, grid, bw0.A * c[3])
            dB <- densityFit(B, grid, bw0.B * c[3])
          }
          dAx <- approx(grid, dA, allObs)$y
          dBx <- approx(grid, dB, allObs)$y
        } else {
          dAx <- densityFit(A, allObs, bw0.A * c[3])
          dBx <- densityFit(B, allObs, bw0.B * c[3])
        }
      }
